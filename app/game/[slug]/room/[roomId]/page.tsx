  ...
  return (
    <GameStateGate slug={slug}>
      ...
      <div className="relative flex min-h-screen flex-col bg-zinc-950">
        ...
        {/* Mobil / tablet: üst HUD — güvenli yatay boşluk */}
        <div className="sticky top-0 z-[60] w-full min-w-0 md:hidden">
          ...
          />
          <div className="border-b border-amber-900/40 bg-zinc-950/95 px-3 pb-2 pt-2 shadow-[0_8px_24px_rgba(0,0,0,0.35)] backdrop-blur-md sm:px-4">
          ...
          <div className="border-b border-amber-900/40 bg-zinc-950/95 px-3 pb-2 pt-1.5 backdrop-blur-md sm:px-4">
            <RoomMap
              slug={slug}
          ...
          <GameRoomLayout
            header={
              <header className="mb-4 flex w-full min-w-0 shrink-0 items-center justify-between gap-4 border-b border-zinc-800/70 bg-zinc-950/92 py-3 backdrop-blur-md sm:mb-5 sm:py-3.5 xl:sticky xl:top-0 xl:z-40">
                <Link
                  href={`/game/${slug}/hub`}
                  className="flex min-h-[44px] min-w-[44px] shrink-0 touch-manipulation items-center justify-center rounded-lg text-sm text-zinc-500 transition-colors hover:text-amber-500 active:bg-zinc-800/50 sm:justify-start sm:px-3"
                >
                  {t.room.back}
                </Link>
